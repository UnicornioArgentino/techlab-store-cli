...
/** --- Handler POST moderno y robusto (preferido por el router) --- */
export async function handlePOST(args){
  const arr = Array.isArray(args) ? args : [args];
  const [resource, title, rawPrice, category] = arr;

  if (!resource) {
    console.error('Falta el recurso. Ej: POST products "Remera" 299 ropa');
    return;
  }

  const price = Number(rawPrice);
  if (!Number.isFinite(price)) {
    console.error('? "price" debe ser numérico. Recibido:', rawPrice);
    return;
  }

  
  if (!title || !category) {
    console.error('Faltan campos: title y category son obligatorios.');
    return;
  }const body = {
    title,
    price,
  ...
  // Log espejo del body que vamos a enviar
  console.log('🔎  Enviando body:\n' + JSON.stringify(body, null, 2));

  // Construir URL absoluta (independiente de utilidades internas)
